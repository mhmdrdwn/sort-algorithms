...
def sort_run_time(list_length, algorithms, cases):
    ...
    -----------
    list_length: list
        list of generated data lengths
    algorithms: list
        sorting algorithms functions
    cases: list
        random, reverse, sorted cases
    """

    np.random.seed(12235)
    for case in cases:
        for len_list in list_length:
            gen_data = np.random.uniform(size=len_list)
            if case == 'sorted_case': 
                test_data = sorted(gen_data)
            elif case == 'reverse_case': 
                test_data = sorted(gen_data, reverse=True)
            else:
                test_data = gen_data
...
def calculate_run_time(algorithm, test_data, len_list, case):
    """
    ...
    len_list: list
        list of lengths of each data generated
    case: string
        string of one case of [random, reverse, sorted]
    """
    ...
    #n_ar = n_ar*5
    run_time_list = np.array(clock.repeat(repeat=3, 
        number=n_ar)) / n_ar
...
def save_run_time(algorithm, len_list, case, run_time, num_of_rep):
    """ 
    the function save all the data to csv file
    
    Parameters:
    -----------
    algorithm: function or method
        sorting algorithm function or method
    len_list: list
        list of lengths of each data generated
    run_time: float
        the recorded running time of sorting
    num_of_rep: int
        number of repetition of running the sort
...
def clean_data(input_file, output_file, list_length, algorithms, cases):
    ...
    algorithms = ['merge_sort', 'combined_sort', 
...
if __name__ == "__main__":
    ...
    header = ['list_length', 'merge_sorted', 'merge_reverse',
        'merge_random', 'combined_sorted', 'combined_reverse',
        'combined_random', 'quick_sorted', 'quick_reverse',
        ...
        'bubble_reverse', 'bubble_random'] 

    sort_run_time(list_length, algorithms, cases)
    write_header(header,'../data/clean_data.csv')
